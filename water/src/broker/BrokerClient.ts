...
export class BrokerClient<T> {
    ...
    protected async sendClusterRequest(
        ...
        const cb: BrokerEventListener<T> = (msg: BrokerMessage<T>) => {
            ...
            messageCallback?.(msg.headers.sourceCluster, msg)
                ?.catch(e => Logger.error(TAG, "Uncaught error in sendClusterRequest message callback", e));
    ...
    public async _respond(msg: BrokerMessage<T>, data: T | null): Promise<void> {
        const newHeaders = this.connection.createHeaders(
            new Set([msg.headers.sourceCluster]),
            msg.headers.requestId,
        );
        await this.send(
            this.toResponseKey(msg.key),
            data,
            newHeaders,
        );
    }
