...
import { BrokerConnection, BrokerMessageHeaders, Logger } from "@";
...
export class RabbitConnection extends BrokerConnection {
	...
	public override async abstractSend(
		...
		if (this.shouldDispatchExternallyAfterShortCircuit(topic, key, value, headers)) {
			...
			if (!publisher) {
				publisher = this.ensureConnection().createPublisher(this.commonPublisherConsumerArguments(topic));
	...
	protected override createTopic(topic: string): void {
		...
		this.consumers.set(
			topic,
			this.ensureConnection().createConsumer(
				{
					queue: queueName,
