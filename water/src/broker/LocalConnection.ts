import type { BrokerMessageHeaders, MessageId } from "@";
...
export class LocalConnection extends BrokerConnection {
	...
	public override async abstractSend(
		topic: string,
		key: string,
		value: string,
		...
		if (this.shouldDispatchExternallyAfterShortCircuit(topic, key, value, headers)) {
			const isForExternalService =
				headers.targetServices.size > 0 && [...headers.targetServices][0] !== this.serviceName;
			const isForExternalInstance =
				headers.targetInstances.size > 0 && [...headers.targetInstances][0] !== this.instanceId;
			if (isForExternalService || isForExternalInstance) {
				throw new Error(
					"Attempting to send message to other services/instances in a LocalConnection " +
